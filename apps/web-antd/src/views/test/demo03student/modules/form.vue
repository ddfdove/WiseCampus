...
const formData = ref<YudaoDemo03StudentApi.Demo03Student>();
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as YudaoDemo03StudentApi.Demo03Student;
    try {
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    // 加载数据
    const data = modalApi.getData<YudaoDemo03StudentApi.Demo03Student>();
    if (!data || !data.id) {
      return;
    }
    modalApi.lock();
